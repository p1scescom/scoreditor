                    ...
                    (dorun (map (fn [b n a] (play-se b n a))
                                (if (>= 1 now-t) (list 0 0 0 0)
                                  (let [ob (.get score-observ (- now-t 2))]
                                    (list (.getA ob) (.getB ob) (.getC ob) (.getD ob))))
  ...
  (letfn [
    ...
    (make-project []
      ...
      (doto @mediaPlayer
        (.setOnReady
          (fn []
            (let [score-files (map #(File. @projectDirectory %) ["easy" "normal" "hard"])
                  score-a (map #(File. % "a") score-files)
                  score-b (map #(File. % "b") score-files)
                  score-c (map #(File. % "c") score-files)
                  score-d (map #(File. % "d") score-files)
                  score-abcds (concat score-a score-b score-c score-d)
                  ;^File score-abcds (map (fn [x] (map (fn [y] (File. x y) ["a" "b" "c" "d"]))) score-files)
                  ]
              ;(spit (File. @projectDirectory "log.txt") score-a score-b score-c score-d)
              (reset! media-duration (.getDuration @media))
              (dorun (map #(.mkdir %) score-files))
              (let [all-zero (str (apply str (repeat (media-fps) \0) ) \4)]
                (dorun (map #(spit % all-zero) score-a))
                (dorun (map #(spit % all-zero) score-b))
                (dorun (map #(spit % all-zero) score-c))
                (dorun (map #(spit % all-zero) score-d))
                #_(dorun (println (str (apply map #(spit % all-zero) score-a score-b score-c score-d))))
                #_(dorun (map #(spit % all-zero) score-abcds)))
                (score-set (first score-files))
                (input-score-set))
            (doto (-> @mediaPlayer .currentTimeProperty)
              (.addListener (proxy [ChangeListener] []
                              (changed [ob ol ne]
    ...
    (score-save []
      (when score-changed
        (when-not (nil? editing-score)
          (let [score-a (str (apply str (map #(.getA %) score-observ)) \4)
    ...
    (menubar-set []
      (let [directoryChooser (DirectoryChooser.)
                            ...
                            (let [f (.showDialog directoryChooser stage)]
                              (when (not (nil? f))
                                ...
                                (when (media-set)
                                  (.setDisable menu-item-new-score false)
                                  (.setDisable menu-item-save-score false)
                                  (score-set (File. @projectDirectory "easy"))
                                  (input-score-set)
                                )))))))
        (doto menu-item-exit
          (.setOnAction (proxy [EventHandler] []
                              ...
                              (when (not= "" score-name)
                                (let [^File score-file (File. @projectDirectory score-name)]
                                  ...
                                  (let [all-zero (apply str (repeat (media-fps) \0))]
                                    (dorun (map #(spit % all-zero) (map #(File. score-file %) ["a" "b" "c" "d"])))
                                    (score-set score-file)
                                    (input-score-set)
                              ))))))))

        (doto menu-item-easy-score
          (.setOnAction (proxy [EventHandler] []
                          (handle [e]
        ...
        (-> menu-score
          .getItems (.addAll [menu-item-easy-score menu-item-normal-score menu-item-hard-score menu-item-save-score]))
                                                 ...
                                                 (proxy [ChangeListener] []
                                                   (changed [ob ol ne]
                                                      (let [poti (-> score-table .getFocusModel .getFocusedCell)
                                                        ...
                                                        (when-not (= cul timeCul)
                                                          (when (<= 0 row)
                                                              (let [tar (.getCellObservableValue cul row)]
                                                          (schange)
                                                          (.set tar (if (or (= (.get tar) @note-mode) (and (= @note-mode 1) (= 3 (.get tar)))) 0 @note-mode))
                                                          (when-not (nil? @editing-score)
                                                          (let [notes (list (.getCellObservableValue aCul row) (.getCellObservableValue bCul row)(.getCellObservableValue cCul row)(.getCellObservableValue dCul row))
                        ...
                        (let [now-time (.getCurrentTime @mediaPlayer)
                          ...
                          (letfn [(print-note [color place zikan]
                                             ...
                                             (doto gc
                                               ...
                                               (doto gc
                                                 (.fillRect (+ (quot c-width 16) (* place (quot c-width 4))) #_(/ c-height 3) (* 0.90 c-height mm) (quot c-width 8) (quot (* 0.925 c-height ) frame-num))))))]
                          (dorun (map (fn [n] (when (> (.size score-observ) n)
